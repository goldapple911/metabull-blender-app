...
import ast# Auto-Rig Pro implant
from collections import OrderedDict# Auto-Rig Pro implant
import array
import datetime
import math
import os
...
from ..utils import *# Auto-Rig Pro implant
...
def copy_shape_keys(ob_source, ob_target):
    for kb in ob_source.data.shape_keys.key_blocks:    
        kb_copy = ob_target.shape_key_add(name=kb.name)
        kb_copy.slider_min = kb.slider_min
        kb_copy.slider_max = kb.slider_max
        kb_copy.value = kb.value
        kb_copy.mute = kb.mute
        for v in ob_target.data.vertices:
            try:# unknown error, the last vertex index may be missing from kb.data
                kb_copy.data[v.index].co = kb.data[v.index].co
            except:
                pass
    ...
    for k, v in items:
        ...
        if isinstance(v, str):
            ...
            try:
                ...
                print('Could not export custom prop:', k, v)
        else:
...
def fbx_data_mesh_elements(root, me_obj, scene_data, done_meshes):
    ...
    if uvnumber:
        ...
        for uvindex, uvlayer in enumerate(me.uv_layers):
            ...
            try:# Auto-Rig Pro implant: Some UV layer may be invalid, not ideal but avoid the crash
...
def fbx_data_material_elements(root, ma, scene_data):
    ...
    if "emission_color" in dir(ma_wrap):# backward-compatibility with Blender 2.80
...
def fbx_data_leaf_bone_elements(root, scene_data):
    ...
    for (node_name, _par_uuid, node_uuid, attr_uuid, matrix, hide, size) in scene_data.data_leaf_bones:
        ...
        rot = matrix.to_euler('XYZ')# Auto-Rig Pro fix
...
def fbx_animations_do(scene_data, ref_id, f_start, f_end, start_zero, objects=None, force_keep=False):
    ...
    while currframe <= f_end:
        currframe = round(currframe, 3)
        real_currframe = currframe - f_start if start_zero else currframe
        real_currframe = round(real_currframe, 3)# round frame value because of decimals issues 
        ...
        for sk_name in animdata_shapes:
            anim_shape, me, shape = animdata_shapes[sk_name]
            #anim_shape.add_keyframe(real_currframe, (shape.value * 100.0,))
            # Auto-Rig Pro implant to export baked shape keys
            #   skip invalid actions (e.g. NLA strips)
            action = None
            try:
                action = ref_id[1]   
            except:
                print("Invalid action, do not export shape keys:", ref_id)
                continue
              
            fetched_value_from_dict = 0.0
            if len(action.keys()):
                if "arp_baked_action" in action.keys():#only baked action have shape keys baked data
                    #print(action.name + '|' + sk_name + '|' + str(real_currframe)) 
                    key = action.name + '|' + sk_name + '|' + str(real_currframe)                    
                    fetched_value_from_dict = scene_data.settings.shape_keys_baked_data_dict[key]
                    
            #print("shape key value", fetched_value_from_dict)
            anim_shape.add_keyframe(real_currframe, (fetched_value_from_dict*100.0,))            
        ...
        currframe += bake_step
        currframe = round(currframe, 3)# round frame value because of decimals issues 
    ...
    # Auto-Rig Pro implant, action renaming
    if '|' in name:
        if scene.arp_export_act_name == 'NONE':
            name_split = name.split('|')
            name = name_split[len(name_split)-1]
        elif scene.arp_export_act_name == '_' or scene.arp_export_act_name == '-':
            name = name.replace('|', scene.arp_export_act_name)
...
def fbx_animations(scene_data):
    ...
    if scene_data.settings.bake_anim_use_all_actions:
        ...
        def restore_object(ob_to, ob_from):
            ...
            for p in props:
                # Artell
                # hack to fix the latest Blender 2.8 update
                valid = True
                try:
                    ob_to.is_property_readonly(p)
                except:
                    valid = False
        ...
        for ob_obj in scene_data.objects:
            ...
            for act in bpy.data.actions:
                # Auto-Rig Pro implant for selective actions export                
                save_action = False
                
                if scene.arp_bake_type == 'ACTIONS' and scene.arp_export_separate_fbx:# export each action as single Fbx file
...
def fbx_data_from_scene(scene, depsgraph, settings):
    ...
    for ob_obj in objects:
        ...
        if settings.use_mesh_modifiers or settings.use_triangles or ob.type in BLENDER_OTHER_OBJECT_TYPES or is_ob_material:
            ...
            if not use_org_data:               
                ...
                # Auto-Rig Pro implant
                # set all shape keys to 0 before mesh conversion
                if ob_to_convert.data.shape_keys:
                    for sk in ob_to_convert.data.shape_keys.key_blocks:
                        sk.value = 0.0
                ...
                # Auto-Rig Pro implant 
                #   make sure to preserve the original mesh name
                name_copy = ob.data.name
                ob.data.name += '_ARPEXPORTTEMP'# hacky but no better solutions found. Reverted later after export
                tmp_me.name = name_copy
    ...
    for ma in data_materials.keys():
        ...
        for sock_name, fbx_name in PRINCIPLED_TEXTURE_SOCKETS_TO_FBX:
            tex = None
            try:# Auto-Rig Pro implant, Blender 2.90 doesn't support all inputs
                tex = getattr(ma_wrap, sock_name)
            except:
                pass
    ...
    # Auto-Rig Pro implant, sort bones alphabetically
    sorted_dict = OrderedDict()
    sorted_names = sorted([i.name for i in data_bones])
    for i in sorted_names:
        for j in data_bones:
            if j.name == i:
                sorted_dict[j] = data_bones[j]
                break    
  
    for bo_obj in sorted_dict.keys():
...
def save_single(operator, scene, depsgraph, filepath="",
                ...
                shape_keys_baked_data=None,# Auto-Rig Pro implant
                export_action_only='all_actions',# //
                **kwargs
                ):
    ...
    shape_keys_baked_data_dict = ast.literal_eval(shape_keys_baked_data)
